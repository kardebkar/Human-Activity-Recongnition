import Foundation
  ...
  final class MacUdpRelay: NSObject, ObservableObject {
      ...
      @Published var macPort: Int = 5500
      ...
      func startRelay() {
          guard !macHost.isEmpty else { lastError = "Set Mac host IP first."; return }
          guard (1...65535).contains(macPort) else { lastError = "Port must be 1–65535.";
  return }
          guard let port = NWEndpoint.Port(rawValue: UInt16(macPort)) else { lastError =
  "Invalid port."; return }

          let conn = NWConnection(host: NWEndpoint.Host(macHost), port: port, using: .udp)
          conn.stateUpdateHandler = { [weak self] state in
              DispatchQueue.main.async {
                  if case .failed(let err) = state { self?.lastError =
  err.localizedDescription }
      ...
      func sendLabelToWatch() {
          ...
          WCSession.default.sendMessage(["label": currentLabel], replyHandler: nil) { [weak
  self] error in
      ...
      func sendCommandToWatch(_ cmd: String) {
          guard WCSession.default.activationState == .activated else { return }
          WCSession.default.sendMessage(["cmd": cmd], replyHandler: nil) { [weak self] error
  in
              DispatchQueue.main.async { self?.lastError = error.localizedDescription }
          }
      }
  ...
  WCSessionActivationState, error: Error?) {
          if let error { DispatchQueue.main.async { self.lastError =
  error.localizedDescription } }
      }

      func session(_ session: WCSession, didReceiveMessageData messageData: Data) {
          guard isRelaying, let conn = connection else { return }
